...
products.each do |product|
  (1..17).each do |i|
    product_webp = URI.parse("https://helios-seeds.s3.us-west-1.amazonaws.com/#{product.sku}-#{i < 10 ? ('0' + i.to_s ) : i}.webp")
    product_webm = URI.parse("https://helios-seeds.s3.us-west-1.amazonaws.com/#{product.sku}-02.webm")
    product_jpeg = URI.parse("https://helios-seeds.s3.us-west-1.amazonaws.com/#{product.sku}-#{i < 10 ? ('0' + i.to_s ) : i}.jpeg")

      begin
        product.photos.attach(
          io: product_webp.open, 
          filename: "#{product.sku}-#{i < 10 ? ('0' + i.to_s ) : i}.webp"
        )
        puts "successfully attached webp to #{product.sku}"
      rescue
        begin
            product.photos.attach(
              io: product_webm.open, 
              filename: `#{product.sku}-02.webm`
            )
            puts("successfully attached webm to #{product.sku}")
        rescue
          begin
            product.photos.attach(
              io: product_jpeg.open, 
              filename: "#{product.sku}-#{i < 10 ? ('0' + i.to_s ) : i}.jpeg"
            )
            puts("successfully attached jpeg to #{product.sku}")
          rescue
            puts "failed to attach jpeg"
          end
        end
      end
  end
